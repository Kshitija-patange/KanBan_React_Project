        ...
        async componentDidMount(){
                TaskService.fetchAllTasks().then((res)=>{
                  ...
                  for (i = 0; i < arr.length; i++){
                        if(!memTemp.includes(arr[i].teamMemberEntity)){
                                memTemp.push(res.data[i].teamMemberEntity);
                        }
                      }
                this.setState({members: memTemp});
                console.log(arr, this.state.members, res.data, memTemp);
